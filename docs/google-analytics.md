...
Each ordered direction is queried independently because a closed `A -> B` funnel can be zero while `B -> A` is nonzero. The `limit` input selects how many top hosts (by sessions) to pair — default **6**, clamped to **12** — so API usage remains bounded at N×(N−1) funnel calls.
